...
```
1、序列化：把内存中的对象（比如一个Java类实例、一个Python字典）转换成字节流或字符串，以便存储到文件、数据库，或者在网络上传输。
反序列化：把上述字节流/字符串还原成内存中的对象。

序列化和反序列化本质是实现数据通信，把类转换成为字节
```

```
2. 漏洞是如何产生的？
漏洞的根本原因：程序没有验证反序列化的数据来源。攻击者把一个精心构造的恶意数据提供给程序，程序反序列化时，对象内部的逻辑被意外触发，造成严重后果。

核心原理：魔术方法（Magic Methods）
很多面向对象语言在对象的生命周期里有一些自动执行的方法，比如创建、销毁、当属性被访问时等。攻击者会寻找这些方法里是否有危险操作（如执行系统命令、读写文件）。
```
...
```
共同条件继承Serializable
入口类source（重写readObject 参数类型宽泛最好jdk自带）
调用链gadget chain
执行类sink  (rcessrf写文件等等）
```

### 共同条件：继承 Serializable (或 Externalizable)

```java
Java：必须实现 java.io.Serializable 或 java.io.Externalizable 接口。如果类没有实现ObjectInputStream.readObject() 会直接抛出 java.io.NotSerializableException。
    
PHP：类可以序列化（通过 serialize()），但只有 __sleep()/__wakeup() 等方法会执行，不强制继承特定类，但反序列化时类必须存在。
    
Python：pickle 不强制继承特定基类，但大部分需要序列化的对象都是普通类实例。
   
-------核心原因：JVM 在反序列化时会检查类的 Serializable 标记。没有这个标记，根本不给你反序列化的机会。所以任何可能被反序列化的类（source、中间 gadget、sink）都必须直接或间接实现 Serializable，否则无法被流化。
--------Serializable 标记在 Java 中本质上是一个空接口（Marker Interface，也叫标记接口），它本身不包含任何方法，但它向 JVM 和编译器传递了一个明确的元信息：“这个类的对象可以被序列化”。
```

### 入口类source（重写readObject 参数类型宽泛最好jdk自带）

```java
当程序调用 ObjectInputStream.readObject() 时，反序列化过程会：

1、读取类描述符
2、创建对象实例
3、调用该类的 readObject() 方法（如果存在）

攻击者需要一个触发点，让反序列化过程自动执行自己的代码或调用其他危险方法。
    
为什么必须重写 readObject？
readObject() 是一个钩子方法，在反序列化时自动被调用。如果类没有重写它，则默认行为只是恢复字段值，不会触发任何额外操作。

重写后：攻击者在 readObject 中放置调用其他 gadget 的代码，从而启动整个链。
    
为什么参数类型要宽泛？
    
攻击者无法控制反序列化时传入的具体对象类型，但可以控制对象的字段。为了让 readObject 中的方法调用能够处理各种类型的恶意对象，参数最好声明为：

接口类型（如 Map、List、Transformer）
父类类型（如 Object）
```

```
为什么最好 JDK 自带？
通用性：JDK 自带类存在于每个 Java 环境中，攻击者不需要依赖第三方库即可利用漏洞。

绕过黑名单：很多反序列化过滤器（如 resolveClass）会优先黑名单第三方库类，但对 JDK 核心类往往不敢随便禁（会导致程序崩溃）。
```

**一句话总结**：入口类是**扳机**，`readObject` 是**扳机扣动的机械结构**，宽泛类型+JDK自带保证了**弹药的通用性和可用性**。

## 2. 调用链 (Gadget Chain) —— 串联入口到执行 相同名字 相同类型

```
为什么需要调用链？
入口类的 readObject 通常不会直接执行 Runtime.exec()（否则太明显，很容易被审计和黑名单拦截）。大多数情况下，readObject 里写的是一些看似无害的操作，比如：
...
这些操作本身不危险，但可以通过精心构造的对象组合，让这些“安全”的方法间接调用到危险方法。

如何形成链条？
基本原理：每个 gadget 是一个类，它内部的某个方法（不限于 readObject）会调用另一个 gadget 的方法，层层嵌套，最终到达 sink。
```

**经典 Java 链例子**（CC1 链简化版）：

```java
AnnotationInvocationHandler.readObject()
  → memberValues.entrySet()          // memberValues 是 LazyMap
    → LazyMap.get()                  // LazyMap.get 会调用 factory.transform()
      → ChainedTransformer.transform() // 循环调用多个 Transformer
        → InvokerTransformer.transform()
          → Runtime.exec()           // 最终 RCE
```

```
为什么不能从 readObject 直接跳到 exec？

防御机制：现在很多反序列化过滤器会检测 Runtime.exec 的直接调用。

灵活性：调用链可以复用不同类库里的 gadget，适应不同环境（比如没有 CC 库，可以用 BeanUtils 或 Jackson）。

绕过限制：某些方法受访问控制限制（private/protected），需要通过反射或代理链来调用。

什么类适合做中间 gadget？
有参数为任意对象类型的方法（如 invoke()、transform()、get()）

接受并调用传入对象的某个方法（如 toString()、hashCode()、getClass()）

传递调用结果（如 ChainedTransformer 把一个 transformer 的输出传给下一个）

一句话总结：调用链是放大器和伪装层，把无害操作变成能执行代码的危险操作。
...
```
为什么需要独立的执行类？
执行类是攻击的终点，负责产生真实的恶意行为。分开设计的原因：

可替换性：根据目标环境选择不同的 sink。

要 RCE → Runtime.exec() 或 ProcessBuilder

要文件写入 → FileOutputStream

要 DNS 外带数据 → InetAddress.getByName() (DNS 请求)

要内存马 → 动态类加载 ClassLoader.defineClass()

组合复用：同一个调用链可以配合不同的 sink。

规避检测：如果 sink 被检测，可以换一个调用链到达不同的 sink。
```
